...
To develop bitcore-node-wei:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-wei.git
...
We now will setup symlinks in `bitcore-node-wei` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-wei
ln -s ~/bitcoind-rpc-wei
```
...
cd devnode
mkdir node_modules
touch bitcore-node-wei.json
touch package.json
```

Edit `bitcore-node-wei.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-wei/bin/bitcore-node-wei start
```