...
var BASE_PACKAGE = {
  description: 'A full Wei node build with Weicore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    '@weicrypto/weicore-lib': '^' + weicore.version,
    '@weicrypto/weicore-node': version
  }
};

/**
 * Will create a directory and wei.conf file for Wei.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createWeiDirectory(datadir, done) {
...
/**
 * Will create a base Weicore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The wei database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/weicore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Weicore Node directory, configuration file,
 * wei configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the weicore node configuration directory
 * @param {String} options.datadir - The path to the wei datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the weicore-node directory and configuration
    ...
    function(next) {
      // Setup the Wei directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createWeiDirectory(absDataDir, next);
      } else {
        next();
      }
    },
