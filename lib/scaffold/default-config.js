...
/**
 * Will return the path and default weicore-node configuration. It will search for the
 * configuration file in the "~/.weicore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.weicore');
  var defaultConfigFile = path.resolve(defaultPath, './weicore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['weid', 'web'];
