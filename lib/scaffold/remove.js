...
/**
 * Will remove a service from weicore-node.json
...
/**
 * Will remove the Node.js service and from the weicore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The weicore-node configuration directory
...
function remove(options, done) {
  ...
  var weicoreConfigPath = path.resolve(configPath, 'weicore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(weicoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a weicore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to weicore-node.json
        removeConfig(weicoreConfigPath, service, next);
      });
    }, done
  );
}
