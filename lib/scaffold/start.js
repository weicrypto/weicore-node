...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for weid is necessary for this upgrade with the "reindex=1" wei.conf option. \n' +
                 'There are changes necessary in both wei.conf and weicore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/weicrypto/weicore-node/blob/weid/docs/upgrade.md \n');
        ...
        servicesConfig: {
          weid: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/weid')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed weicore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './weicore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in weicore-node services directory
    ...
    if (servicePackage.weicoreNode) {
      serviceModule = service.name + '/' + servicePackage.weicoreNode;
    }
 ...
 * @param {Node} node - The Weicore Node instance
 */
