...
var Proposal = weicore.GovObject.Proposal;
...
/**
 * Provides a friendly event driven API to weid in Node.js. Manages starting and
 * stopping weid as a child process for application support, as well as connecting
 * to multiple weid processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Wei(options) {
  if (!(this instanceof Wei)) {
    return new Wei(options);
  }
...
util.inherits(Wei, Service);

Wei.dependencies = [];

Wei.DEFAULT_MAX_TXIDS = 1000;
Wei.DEFAULT_MAX_HISTORY = 50;
Wei.DEFAULT_MAX_UTXO = 1000;
Wei.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Wei.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Wei.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Wei.DEFAULT_SPAWN_RESTART_TIME = 5000;
Wei.DEFAULT_SPAWN_STOP_TIME = 10000;
Wei.DEFAULT_TRY_ALL_INTERVAL = 1000;
Wei.DEFAULT_REINDEX_INTERVAL = 10000;
Wei.DEFAULT_START_RETRY_INTERVAL = 5000;
Wei.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Wei.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Wei.DEFAULT_INSTANTSEND_FEE = 10000;
Wei.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Wei.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Wei.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Wei.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Wei.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Wei.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Wei.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Wei.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Wei.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Wei.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Wei.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Wei.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Wei.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Wei.prototype._initCaches = function() {
...
Wei.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Wei.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Wei.prototype.getPublishEvents = function() {
  return [
    {
...
Wei.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'weid/' + name, 'total:', this.subscriptions[name].length);
};

Wei.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'weid/' + name, 'total:', this.subscriptions[name].length);
};

Wei.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (weicore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'weid/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Wei.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'weid/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Wei.prototype.unsubscribeAddressAll = function(emitter) {
...
Wei.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Wei.DEFAULT_CONFIG_SETTINGS;
...
Wei.prototype._parseWeiConf = function(configPath) {
  var options = {};
...
Wei.prototype._expandRelativeDatadir = function() {
...
Wei.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in weid config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in weid config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in weid config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './wei.conf');

  log.info('Using Wei config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseWeiConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseWeiConf(networkConfigPath));
  }
...
Wei.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
      spawnConfig.txindex && spawnConfig.txindex === 1,
      '"txindex" option is required in order to use transaction query features of weicore-node. ' +
  ...
  $.checkState(
      ...
      '"addressindex" option is required in order to use address query features of weicore-node. ' +
  ...
  $.checkState(
      ...
      '"spentindex" option is required in order to use spent info query features of weicore-node. ' +
  ...
  $.checkState(
      ...
      '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in wei.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that weid is undergoing a reindex. ' +
        ...
        'of weicore-node services will start.');
    node._reindex = true;
  }
};

Wei.prototype._resetCaches = function() {
 ...
 * @private
 */
Wei.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Wei.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Wei.prototype._initChain = function(callback) {
  var self = this;
...
Wei.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === weicore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Wei.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === weicore.Networks.testnet) {
    networkPath = 'testnet3/wei.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/wei.conf';
    }
  }
  return networkPath;
};

Wei.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === weicore.Networks.testnet) {
...
Wei.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('weid/hashblock', message.toString('hex'));
    }
  }

};

Wei.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Wei.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Wei Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Wei.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Wei.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('weid/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Wei.prototype._zmqTransactionHandler = function(node, message) {
  ...
  var self = this;
  var hash = weicore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('weid/rawtransaction', message.toString('hex'));
    }

    var tx = weicore.Transaction();
    tx.fromString(message);
    var txid = weicore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Wei.prototype._zmqTransactionLockHandler = function (node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('weid/transactionlock', message.toString('hex'));
    }

  }
};

Wei.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Wei.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Wei.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Wei.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Wei.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Wei Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Wei.DEFAULT_REINDEX_INTERVAL);

};

Wei.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Wei.prototype._stopSpawnedWei = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/weid.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned wei process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean wei process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Wei.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedWei(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting wei process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Wei process unexpectedly exited with code:', code);
        log.warn('Restarting wei child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Wei process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn weid.'));
      }
...
Wei.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to weid.'));
    }
 ...
 */
Wei.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Wei configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Wei.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Wei.prototype.syncPercentage = function(callback) {
  var self = this;
...
Wei.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, weicore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Wei.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wei.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = weicore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Wei.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Wei.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Wei.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Wei.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in weicore is most recent to less recent
 ...
 * @param {Function} callback
 */
Wei.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Wei.prototype._getConfirmationsDetail = function(transaction) {
...
Wei.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Wei.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Wei.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A wei transaction id
 * @param {Function} callback
 */
Wei.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Wei.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof weicore.Address) {
...
Wei.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Wei.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Wei.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Wei.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wei.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Wei.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Weicore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Wei.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = weicore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Wei.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wei.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Wei.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
  var self = this;
  var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Wei.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wei.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Wei.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Wei.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Weicore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Wei.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wei.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param callback
 */
Wei.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Wei.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Wei.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Wei.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Wei.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Wei.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Wei.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Wei.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Wei.prototype.govCount = function (callback) {
  var self = this;
...
Wei.prototype.getSpork = function(callback){
  var self = this;
  var SporksList = {};
...
Wei.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Wei.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Wei.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Wei.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wei.prototype.getBestChainLock = function (callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wei.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wei.prototype.getInfo = function(callback) {
  var self = this;
...
Wei.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Wei.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('weid spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('weid process did not exit'));
      }
